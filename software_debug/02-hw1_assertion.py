        ...
        if event == "call" or event == "return":
            ...
            if event == 'return':
                if 'ret' not in self.vars[func_name][event]:
                    r = Range()
                    r.track(arg)
                    self.vars[func_name][event]['ret'] = r
                else:
                ...
                for var in loc.keys():
                    if var not in self.vars[func_name][event]:
                        r = Range()
                        r.track(loc[var])
                        self.vars[func_name][event][var] = r
                    else:
                        self.vars[func_name][event][var].track(loc[var])
...
# For this input the expected result should contain the following lines:
#
