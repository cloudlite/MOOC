...
# This value will indicate which return value we are testing for
mode = None

# This will hold the max coefficient
...
def traceit(frame, event, arg):
    ...
    if mode == -1:
        ...
        if event == "return" and (
                    (isinstance(arg, str) == False and arg == 0) or (isinstance(arg, str) == True and len(arg) == 0)):
            ok_to_add = True
    elif mode == 1:
        if event == "return" and (
                    (isinstance(arg, str) == False and arg > 0) or (isinstance(arg, str) == True and len(arg) > 0)):
    ...
    if ok_to_add == True:
        ...
        coverage[filename][frame.f_code.co_name] = True
...
def print_tables(tables):
    ...
    for filename in tables.keys():
        ...
        for i in range(44, 89):  # lines of the remove_html_markup in this file
    ...
    funcs = ["f1", "f2", "f3"]
    for func in funcs:
        if tables[filename].has_key(func):
            (n11, n10, n01, n00) = tables[filename][func]
            try:
                factor = phi(n11, n10, n01, n00)
                prefix = "%+.4f%2d%2d%2d%2d" % (factor, n11, n10, n01, n00)
            except:
        ...
        if max_coefficient == None:
            max_coefficient = factor
            max_function = func
            max_mode = mode
        elif factor > max_coefficient:
            max_coefficient = factor
            max_function = func
            max_mode = mode
    ...
    for (input, outcome, coverage) in runs:
        for filename, lines in coverage.iteritems():
            for line in lines.keys():
                ...
                if not tables[filename].has_key("f1"):
                    tables[filename]["f1"] = (0, 0, 0, 0)
                if not tables[filename].has_key("f2"):
                    tables[filename]["f2"] = (0, 0, 0, 0)
                if not tables[filename].has_key("f3"):
                    tables[filename]["f3"] = (0, 0, 0, 0)
...
# Get values for greater than zero
print "Run for greater than zero:"
mode = 1
...
print "answer_function: " + max_function
print "answer_bin: " + repr(max_mode)
print "answer_value: " + repr(round(max_coefficient, 4))